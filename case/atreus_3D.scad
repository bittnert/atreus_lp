...
module screw_hole(radius, offset_radius, position, direction) {
  ...
  translate([x,y,0]) {
    cylinder(r1=radius,r2=radius,h=4);
  }
}
...
module pcb_cutout() {
    ...
    translate([-pcb_width/2, staggering_offsets[n_cols-1] + n_rows * column_spacing - 14.5 , 0])
    cube([pcb_width, pcb_length, pcb_height]);
    translate([-4.5,staggering_offsets[n_cols-1] + n_rows * column_spacing,0])
    #cube([9, 100, pcb_height]);
}

module battery_cutout(){
    battery_width = 26; //We have 13 mm to both sides, battery will not be 26 mm width
    
    translate([-battery_width/2, staggering_offsets[n_cols-1] + n_rows * column_spacing - 14.5 - 30, 0])
    cube([battery_width, 30, 3.5]);
}
